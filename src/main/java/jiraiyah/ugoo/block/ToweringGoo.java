...
public class ToweringGoo extends BlockWithEntity
{
    public static final MapCodec<ToweringGoo> CODEC = createCodec(ToweringGoo::new);
    ...
    {
        return new ToweringGooBlockEntity(pos, state);
    }

    public <T extends BlockEntity> @Nullable BlockEntityTicker<T> getTicker(World level, BlockState state, BlockEntityType<T> type) {
    return validateTicker(type, ModBlockEntities.TOWERING_GOO, ToweringGooBlockEntity::tick);
}
}