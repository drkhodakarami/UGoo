...
public class LavaGeneratingGoo extends BlockWithEntity
{
    public static final MapCodec<LavaGeneratingGoo> CODEC = createCodec(LavaGeneratingGoo::new);
    ...
    {
        return new LavaGeneratingGooBlockEntity(pos, state);
    }

    public <T extends BlockEntity> @Nullable BlockEntityTicker<T> getTicker(World level, BlockState state, BlockEntityType<T> type) {
        return validateTicker(type, ModBlockEntities.LAVA_GENERATING_GOO, LavaGeneratingGooBlockEntity::tick);
    }
}