...
public class BridgeGoo extends BlockWithEntity
{
    public static final MapCodec<BridgeGoo> CODEC = createCodec(BridgeGoo::new);
    ...
    {
        return new BridgeGooBlockEntity(pos, state);
    }

    public <T extends BlockEntity> @Nullable BlockEntityTicker<T> getTicker(World level, BlockState state, BlockEntityType<T> type) {
        return validateTicker(type, ModBlockEntities.BRIDGE_GOO, BridgeGooBlockEntity::tick);
    }
}